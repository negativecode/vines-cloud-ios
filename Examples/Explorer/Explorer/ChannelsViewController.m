...
- (void)viewDidLoad
{
    ...
    [appDelegate.appLoaded done:^(NSObject *found) {
        ...
        [app channels:^(NSMutableArray *rows, VCError *error) {
            self.channels = rows;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    VCChannel *channel = [self.channels objectAtIndex:indexPath.row];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"ViewChannel"]) {
        ChannelViewController *controller = segue.destinationViewController;
        VCChannel *channel = [self.channels objectAtIndex:self.tableView.indexPathForSelectedRow.row];
        controller.title = channel.name;
        controller.channel = channel;
    } else if ([segue.identifier isEqualToString:@"AddChannel"]) {
        UINavigationController *nav = segue.destinationViewController;
        AddChannelViewController *controller = [nav.viewControllers objectAtIndex:0];
...
- (void)addChannelViewController:(AddChannelViewController *)controller didAddChannel:(NSString *)channelName
{
    ...
    VCChannel *channel = [delegate.vinesApp channelForName:channelName];
    [self.channels addObject:channel];

    NSIndexPath *path = [NSIndexPath indexPathForRow:[self.channels count] - 1 inSection:0];
